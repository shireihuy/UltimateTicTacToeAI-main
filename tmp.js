            ...
            for(var b = 0; b < 9; b++){
                if(checkWinCondition(boards[currentBoard]) === 0){
                    if (boards[currentBoard][b] === 0) {
                        ...
                        if (MOVES < 20) {
                            savedMm = miniMax(boards, b, Math.min(maxDepth, count), -Infinity, Infinity, false);
                        } else if (MOVES < 32) {
                            console.log("DEEP SEARCH");
                            savedMm = miniMax(boards, b, Math.min(maxDepth + 1, count), -Infinity, Infinity, false);
                        } else {
                            console.log("ULTRA DEEP SEARCH");
                            savedMm = miniMax(boards, b, Math.min(maxDepth + 2, count), -Infinity, Infinity, false);
                        }
                        console.log(savedMm);
...
function startGame(type, difficultyLevel = 6) {
    boards = [
    ...
    if(type === 0){
        ...
        playerNames[1] = "AI";
        maxDepth = difficultyLevel; // Set the maximum depth for the minimax search
    }else{
        AIACTIVE = false;
        gameRunning = true;
        switchAroo = 1;
